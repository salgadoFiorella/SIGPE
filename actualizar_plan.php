...
<?php
  ...
  require_once('includes/load.php');
	...
	page_require_permission('modificar_plan');
?>
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
...
#line{
  ...
  <?php include_once('layouts/theader.php'); ?>
    ...
    $sqlAreaD = "SELECT *  FROM areas_disciplinarias where planAct='S' and planId like '$toggle' ";
    ...
    $queryDOCS= 'SELECT * FROM documento WHERE planAct="Y" and planId ='.$toggle;
    $rsDOCS = mysqli_query($conn,$queryDOCS);
...
<h1>Modificar Plan <?php echo $nombre;?></h1>
<hr>
...
<br>
<form  id="msform" method="post" enctype="multipart/form-data" action="agregarDatosAlModificar.php" name="datos">
        ...
        <div class="card-body">
					<div class=" form-group input-group input-group-lg">
							<div class="col-md-6"><!-- 1-->
								<h6>Código de Registro</b></h6>
									<input class="form-control" id="c" type="text" name="codigoRegistro" required="" value="<?php echo $reg;?>">
							</div> <!-- cierra el col-md-6 1-->
							<div class="col-md-6"> <!--2-->
								<h6>Código Banner<b style="color:red;">*</b></h6>
									<input class="form-control" id="d" type="text" name="codigoBanner" required="" value="<?php echo $ban;?>">
							</div> <!-- cierra el col-md-6 2-->
					</div><!-- cierra el form-group input-group-->
					<div class=" form-group input-group input-group-lg">
						<div class="col-md-6">
							<h6>Código Banner #2</h6>
							<input class="form-control" id="d" type="text" name="codigoBanner2" value="<?php echo $ban2;?>">
						</div>
					</div><!-- cierra el form-group input-group-->
				</div> <!-- cierra el card-body-->
	</div><!-- cierra el div id-->
					...
					<div class="panel panel-primary">
						...
						<?php
							if($resultAc->num_rows > 0){
								...
								echo '<table class="table"><thead><tr><th scope="col">Desde</th><th scope="col">Hasta</th><th scope="col" style="width=100px">Detalle</th></tr></thead><tbody>';
								...
								echo '<input type="text" size="100" class="form-control" name="det-'.$fila['AcredCod'].'" value="'.$fila['Detalle'].'">';
								echo '</td><tr>';
								}
...
<div class="card">
	...
	<div id="collapseThirteen" class="collapse" aria-labelledby="headingThirteen" data-parent="#accordion">
		...
		<h6 style="color: #428bca;">**Si no desea modificar los archivos existentes, no cargue ningún otro archivo**</h6>
			...
			<div class="col-7">
			<?php
			echo '<a target="_blank" href=script/descargarExcel.php?file='.$toggle.'>Descargar Prevista Estructura csv</a>';
			?><br><br>
			</div>
			</div>
			...
			<div class="row">
				<div class="col-5">
					...
					<input type="file" id="archcsv" name="archcsv" accept=".xlsx, .xls, .csv" onchange="verifyInputFile()">
				</div><!--col-5-->
				<div class="col-5">
...
<div class="card">
	<div class="card-header" id="headingFourteen">
		...
		14. Documentos Asociados Previamente
		</a>
		</h5>
	</div>
	<div id="collapseFourteen" class="collapse" aria-labelledby="headingFourteen" data-parent="#accordion">
		<div class="card-body">
			<h5>Documentos previamente guardados</h5>
			<h6 style="color: #428bca;">**Deseleccione la casilla en caso de querer eliminar un documento.**</h6><br>
			<?php
                if($rsDOCS->num_rows > 0){
				  $rsDOCS->data_seek(0);
				  echo '<div class="table-responsive">';
                  echo '<table class="table table-hover"><thead><tr><th scope="col">Activo</th><th>Tipo</th><th scope="col">Documento</th></tr>';
                  while($fila=$rsDOCS->fetch_assoc()){
                    echo '<tr><td>';
					echo '<input type="checkbox" id="'.$fila['codigoDoc'].'" name="docsAct[]" value="'.$fila['codigoDoc'].'" checked></td><td>';
					echo '<input size="40" type="text" disabled value="'.$fila['tipoDoc'].'"></td><td>';
                    echo '<a target="_blank" href=script/downloadFile.php?file='.$fila["codigoDoc"].'>
                    '.$fila["nombre"].'
                  </a>';
                    //echo '<input type="text" class="form-control" name="enf-'.$fila['nombre'].'" value="'.$fila['nombre'].'">';
                    echo '</td><tr>';
                  }
                  echo'</tbody></table></div>';
                }else{
					echo "<h4><b>No hay documentos asociados a este plan</b></h4>";
				}?>
		</div> <!-- cierra el card-body-->
	</div><!-- cierra el div id-->
</div> <!-- cierra el card-->
...
$("#btnabrir").click(function(){
	...
	$('#collapseFifteen').addClass('collapse show');
});
    ...
    $('#collapseFifteen').removeClass('collapse show');
	...
	function verifyInputFile(){
		if(document.getElementById("archcsv").value != "") { //verifica si subieron un archivo csv en estructura curricular
			document.getElementById("archcsv").required = true;//Pone el atributo requerido en ambos inputs
			document.getElementById("archpdf").required = true;
		}
	}
	</script>
